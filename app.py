...
def grafico_composicao_ideb(trans, coluna_nome, titulo):
    """
    Cada unidade é uma observação independente.
    A linha curta liga N ao IDEB na mesma escala 0–10.
    P aparece como informação da composição, sem criar eixo incompatível.
    """
    if trans.empty:
        return go.Figure()
    ...
    for _, r in d.iloc[::-1].iterrows():
        ...
        fig.add_trace(go.Scatter(
            ...
            y=[nome],
            mode="markers+text",
            marker=dict(size=14, color=COR_RANK_N),
            text=[f"N {fmt(n,2)}"],
            textposition="top center",
            name="N",
            showlegend=False,
            hovertemplate=f"<b>{nome}</b><br>N: {n:.2f}<extra></extra>"
        ))
        ...
        fig.add_trace(go.Scatter(
            ...
            y=[nome],
            mode="markers+text",
            marker=dict(size=14, color=COR_RANK_IDEB),
            text=[f"IDEB {fmt(ideb,1)}"],
            textposition="bottom center",
            name="IDEB",
            showlegend=False,
            hovertemplate=f"<b>{nome}</b><br>IDEB: {ideb:.1f}<extra></extra>"
        ))
    ...
    fig.update_layout(
        ...
        height=max(330, 95 + 72 * len(d)),
        margin=dict(l=220, r=45, t=75, b=55),
        paper_bgcolor="white",
        plot_bgcolor="white",
        xaxis=dict(
            title="Escala 0–10",
    ...
    def painel_escolas():
        ...
        with t3:
            ...
            if not lista_e1 or not lista_e2:
                ...
                with st.form("form_comp_escolas_v2104"):
