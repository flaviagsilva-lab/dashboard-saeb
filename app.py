...
if pagina == "Visão Geral":
    ...
    def painel_territorio():
        ...
        for etapa in etapas:
            ...
            st.caption(
                "Cada município é independente. As barras mostram N e IDEB na escala 0–10; "
                "a linha mostra P na escala 0–1. A relação apresentada é N × P → IDEB."
            )
            ...
            if trans.empty:
                st.info(
                    "Não há dados suficientes para a composição do IDEB "
                    "no ano selecionado."
                )
            else:
                st.plotly_chart(
                    grafico_composicao_ideb(
                        trans,
                        "Município",
                ...
                with st.expander("Ver valores da composição"):
                    tabela_np = trans.copy()
                    for col, casas in [
                        ("Nota Padronizada LP", 2),
                        ("Nota Padronizada Matemática", 2),
                        ("N", 2),
                        ("P", 3),
                        ("IDEB", 1)
                    ]:
                        tabela_np[col] = tabela_np[col].map(
                            lambda v, c=casas: fmt(v,c) if pd.notna(v) else "—"
                        )
                    st.dataframe(
                        tabela_np,
                        hide_index=True,
