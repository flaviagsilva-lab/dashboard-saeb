...
def carregar_bases(versao_dados="2.10.4"):
...
municipios, escolas, investimento = carregar_bases("2.10.4")
...
def grafico_lp_mat(df, titulo, modo="linhas"):
    ...
    if modo == "linhas":
        ...
        adicionar_rotulos_acima(
            ...
            casas=1, cor=ROSA, yshift=6, xshift=-6
        )
        ...
        adicionar_rotulos_acima(
            ...
            casas=1, cor=LILAS, yshift=6, xshift=6
        )
    else:
...
def grafico_np_ideb(df, titulo):
    ...
    - N em barras (escala 0–10);
    - IDEB em linha (escala 0–10);
    - P em linha no eixo secundário (escala 0–1).
    """
    ...
    fig.add_trace(go.Bar(
        x=d["Ano"],
        y=d["N"],
        name="Nota Média Padronizada (N)",
        marker_color=COR_RANK_N,
        text=[fmt(v,2) if pd.notna(v) else "" for v in d["N"]],
        textposition="outside",
        cliponaxis=False,
        hovertemplate="Ano %{x}<br>N: %{y:.2f}<extra></extra>"
    ))
    ...
    fig.add_trace(go.Scatter(
        ...
        line=dict(color=COR_RANK_IDEB, width=3),
        marker=dict(size=8),
        hovertemplate="Ano %{x}<br>IDEB: %{y:.1f}<extra></extra>"
    ))

    fig.add_trace(go.Scatter(
        x=d["Ano"],
        y=d["P"],
        name="Indicador de Rendimento (P)",
        mode="lines+markers+text",
        yaxis="y2",
        text=[fmt(v,3) if pd.notna(v) else "" for v in d["P"]],
        textposition="bottom center",
        line=dict(color=LARANJA, width=2.5, dash="dot"),
        marker=dict(size=7),
        hovertemplate="Ano %{x}<br>P: %{y:.3f}<extra></extra>"
    ))

    estilo_fig(fig, titulo, "N / IDEB", 520)
    fig.update_layout(
        barmode="group",
        yaxis=dict(
            title="N e IDEB — escala 0–10",
            range=[0, 10],
            dtick=1,
            gridcolor="#EDF1F4"
        ),
        ...
        margin=dict(l=45, r=70, t=80, b=45)
    )
    return fig
...
def grafico_composicao_ideb(trans, coluna_nome, titulo):
    """
    Visualização da composição do IDEB no mesmo padrão gráfico de LP + Matemática × N:
    - barras agrupadas para N e IDEB, ambos na escala 0–10;
    - linha para P no eixo secundário, escala 0–1.
    """
    if trans.empty:
        return go.Figure()
    ...
    d["IDEB"] = pd.to_numeric(d["IDEB"], errors="coerce")
    d = d.dropna(subset=["N", "IDEB"])
    ...
    # N e IDEB compartilham a escala 0–10.
    fig.add_trace(go.Bar(
        x=d[coluna_nome],
        y=d["N"],
        name="Nota Média Padronizada (N)",
        ...
        textposition="outside",
        cliponaxis=False,
        hovertemplate="<b>%{x}</b><br>N: %{y:.2f}<extra></extra>"
    ))
    ...
    fig.add_trace(go.Bar(
        ...
        y=d["IDEB"],
        name="IDEB",
        marker_color=COR_RANK_IDEB,
        text=[fmt(v,1) if pd.notna(v) else "" for v in d["IDEB"]],
        textposition="outside",
        cliponaxis=False,
        hovertemplate="<b>%{x}</b><br>IDEB: %{y:.1f}<extra></extra>"
    ))

    # P é fator de rendimento e usa escala própria de 0 a 1.
    fig.add_trace(go.Scatter(
        x=d[coluna_nome],
        y=d["P"],
        name="Indicador de Rendimento (P)",
    ...
    fig.update_layout(
        title=dict(text=titulo, x=.01, xanchor="left"),
        height=max(430, 390 + 25 * len(d)),
        margin=dict(l=55, r=70, t=85, b=95),
        paper_bgcolor="white",
        plot_bgcolor="white",
        barmode="group",
        hovermode="x unified",
...
def cards_composicao_ideb(trans, coluna_nome):
    ...
    for _, r in trans.iterrows():
        st.markdown(
            ...
            f'<div class="metric-foot">N e P são lidos da base oficial; o painel não recalcula N.</div>'
            f'</div>',
            unsafe_allow_html=True
        )
...
def classificacao_visual_ranking(indicador_label, valor, nivel_saeb=None, padrao_saeb=None):
    ...
    if indicador_label in ["Língua Portuguesa (SAEB)", "Matemática (SAEB)"]:
        if pd.notna(nivel_saeb):
            ...
            etapa = (
                st.session_state.get("rank_m_etapa")
                or st.session_state.get("rank_e_etapa")
                or "Fundamental I"
            )
    ...
    if indicador_label == "IDEB":
        return "IDEB", COR_RANK_IDEB

    if indicador_label in ["Nota Média Padronizada (N)", "N"]:
        return "N", COR_RANK_N

    if indicador_label in [
        "Nota Padronizada LP",
        "Nota Padronizada Matemática"
    ]:
        return "Nota padronizada", "#0E5A70"

    return indicador_label, "#0E5A70"
...
def adicionar_rotulos_acima(
    ...
    """
    Mantém os valores próximos dos respectivos pontos.
    Todos ficam acima; quando necessário, as séries se separam
    horizontalmente para não afastar o número da linha.
    """
...
def grafico_comparativo_referencias_historico(df_barueri, etapa):
    ...
    series = [
        ("Barueri", barueri, cores["Barueri"], -9),
        ("São Paulo", sp, cores["São Paulo"], 0),
        ("Brasil", brasil, cores["Brasil"], 9),
    ]

    for nome, valores, cor, deslocamento_x in series:
        fig.add_trace(go.Scatter(
            x=anos,
            y=valores,
            mode="lines+markers",
...
def grafico_executivo_ideb_rede(fi, fii, etapa_modo="Ambos"):
    fig = go.Figure()

    series = []
    if etapa_modo in ["Fundamental I", "Ambos"]:
        series.append((fi, "Fundamental I", "#0E5A70", -6))
    if etapa_modo in ["Fundamental II", "Ambos"]:
        series.append((fii, "Fundamental II", "#7C3AED", 6))

    for df, etapa, cor, deslocamento_x in series:
        d = df.copy().sort_values("Ano")
    ...
    titulo_etapa = {
        ...
        "Ambos": "Fundamental I e Fundamental II"
    }[etapa_modo]

    estilo_fig(
        fig,
        f"Evolução do IDEB e metas — {titulo_etapa}",
        "IDEB",
        540
    )
...
if pagina == "Visão Geral":
    ...
    if len(anos_fi) >= 2 and len(anos_fii) >= 2:
        ...
        st.plotly_chart(
            grafico_executivo_ideb_rede(fi, fii, etapa_visao),
            use_container_width=True,
    ...
    def painel_escolas():
        ...
        with t1:
            ...
            st.markdown(
                '<div class="section-title">Composição do IDEB — evolução histórica</div>',
                unsafe_allow_html=True
            )
            st.caption(
                "A leitura é temporal para a mesma escola: N e IDEB na escala 0–10 "
                "e P no eixo secundário de 0–1. A relação é N × P → IDEB."
            )
            st.plotly_chart(
                grafico_np_ideb(
                    d,
        ...
        with t3:
            ...
            if not lista_e1 or not lista_e2:
                ...
                with st.form("form_comp_escolas_v2105"):
    ...
    def painel_aprendizagem():
        ...
        # ====================================================
        with abas_apr[0]:
            etapa_visao_m = st.segmented_control(
                "Etapa",
                ["Fundamental I", "Fundamental II", "Ambos"],
            ...
            etapas_m = (
                ["Fundamental I", "Fundamental II"]
                if etapa_visao_m == "Ambos"
                else [etapa_visao_m]
            )

            for etapa in etapas_m:
                base = dados_municipio("Barueri", etapa, rede="Municipal")
                anos_disp = sorted(int(a) for a in base["Ano"].dropna().unique())

                if not anos_disp:
                    st.info(f"Sem dados disponíveis para {etapa}.")
                    continue

                if etapa_visao_m == "Ambos":
                    st.markdown(
                        f'<div class="section-title">{etapa}</div>',
                        unsafe_allow_html=True
                    )

                ano = st.selectbox(
                    f"Ano de referência — {etapa}",
                    anos_disp,
                    index=len(anos_disp)-1,
                    key=f"apr_ano_rede_{etapa}"
                )
                ...
                if row is not None:
                    ...
                    with c2:
                        st.plotly_chart(
                            fig_escala_saeb(
                                row, etapa, "Matemática"
                            ),
                            use_container_width=True
                        )

                    st.markdown(
                        '<div class="section-title">O que este resultado indica?</div>',
                        unsafe_allow_html=True
                    )
                    c1, c2 = st.columns(2)

                    lp_padrao = localizar_padrao(
                        row.get("Língua Portuguesa"),
                        "Língua Portuguesa",
                        etapa
                    )
                    mat_padrao = localizar_padrao(
                        row.get("Matemática"),
                        "Matemática",
                        etapa
                    )
                ...
                st.caption(
                    "Aqui a comparação é temporal para o mesmo município. "
                    "N e IDEB usam a escala 0–10; P aparece no eixo secundário de 0–1. "
                    "A relação é N × P → IDEB."
                )
                st.plotly_chart(
                    grafico_np_ideb(
                        base,
                        f"N × P → IDEB — Barueri — {etapa}"
                    ),
                    use_container_width=True,
                    config={
                        "displaylogo": False,
        ...
        with abas_apr[2]:
            ...
            # ------------------------------------------------
            with st.form("form_ranking_municipios_v2100"):
    ...
    def painel_territorio():
        etapa_sel = st.segmented_control(
            "Etapa",
            ["Fundamental I", "Fundamental II", "Ambos"],
            default="Fundamental I",
            selection_mode="single",
        ...
        filtro_etapa = municipios["Etapa"].isin(etapas)
        ...
        anos_por_etapa = []
        for etapa in etapas:
            b = dados_municipio("Barueri", etapa, rede="Municipal")
            anos_por_etapa.append(
                set(int(a) for a in b["Ano"].dropna().unique())
            )

        anos_disp = sorted(
            set.intersection(*anos_por_etapa)
            if len(anos_por_etapa) > 1
            else anos_por_etapa[0]
        )

        with st.form("form_comparacao_municipio_v2106"):
            c1, c2 = st.columns(2)
            with c1:
        ...
        for etapa in etapas:
            ...
            st.markdown(
                ...
                f'<b>Municípios com registro em {ano_fim}:</b> '
                f'{total_rede_territorio}.</div>',
                unsafe_allow_html=True
            )

            st.markdown(
                '<div class="section-title">Língua Portuguesa e Matemática</div>',
                unsafe_allow_html=True
            )
            ...
            st.plotly_chart(
                grafico_comparacao_indicador(
                    datasets,
                    indicador,
                    f"{indicador} — Barueri e municípios selecionados — {etapa}"
                ),
                use_container_width=True
            )
